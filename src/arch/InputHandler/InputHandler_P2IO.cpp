...
for your p2io. Yup... somehow the winusb approximation library actually works with this... go figure
...
this depends on io/p2io.ccp/h -- see there for additional details. Sadly libusb based approach is NOT portable to sm5. Someone want to port to windows fileio?
...
#include <array>
#endif
REGISTER_INPUT_HANDLER_CLASS(P2IO);
#else
REGISTER_INPUT_HANDLER(P2IO);
#endif

bool InputHandler_P2IO::s_bInitialized = false;
uint8_t InputHandler_P2IO::usb_light_message[] = { 0, 0, 0, 0, 0 };

static Preference<PSTRING> m_sCabinetType("CabinetType", "SD");



//This is the old p2io -> UDP network packet definition... nothing special to see here folks... probably leave this alone...if it aint broke dont fix it!
...
#ifdef PRODUCT_ID_BARE
void InputHandler_P2IO::GetDevicesAndDescriptions(std::vector<InputDeviceInfo>& vDevicesOut)
{
	if (m_bFoundUSBDevice)
	{
		vDevicesOut.push_back(InputDeviceInfo(DEVICE_KEYBOARD, "P2IO"));
	}
}

#ifndef STDSTRING_H //rstring doesnt exist...
std::array<DeviceButton, P2IO_NUM_STATES> button_list_dupe =
{
#else //  STDSTRING_H
const DeviceButton InputHandler_P2IO::button_list[P2IO_NUM_STATES] =
#endif

#else
void InputHandler_P2IO::GetDevicesAndDescriptions(vector<InputDevice>& vDevicesOut, vector<PSTRING>& vDescriptionsOut)
{
	if (m_bFoundUSBDevice)
	{
		vDevicesOut.push_back(InputDevice(DEVICE_KEYBOARD));
		vDescriptionsOut.push_back("P2IO");
	}
}
const int InputHandler_P2IO::button_list[P2IO_NUM_STATES] =
#endif
{
	//player 1 pad
...
InputHandler_P2IO::InputHandler_P2IO()
{
	m_bFoundUSBDevice = false;

	LOG->Info("Checking P2IO cabinet type...");
	...
	{
		HDCabinet = true;
		LOG->Info("P2IO Cabinet Type set to HD.");
	}
	else
	{
		LOG->Info("P2IO Cabinet Type set to SD.");
	}


	for (int i = 0; i<P2IO_NUM_STATES; i++)
	{
		previousStates[i] = 0;
	}
	...
	* and it has claimed the board, don't try to claim it again. */
	...
	if (s_bInitialized)
	{
		LOG->Warn("InputHandler_P2IO: Redundant driver loaded. Disabling...");
		return;
	}
	...
	if (!Board.Open())
	{
		LOG->Warn("InputHandler_P2IO: could not establish a connection with the I/O device.");
		return;
	}

	LOG->Info("Opened P2IO USB board.");
	...
	USBThread.SetName("P2IO USB Interrupt thread");
	USBThread.Create(USBThread_Start, this);

	USBBulkThread.SetName("P2IO USB Bulk thread");
	
	LOG->Info("detect P2IO bulk thread.");
...
InputHandler_P2IO::~InputHandler_P2IO()
{
	...
	{
		LOG->Trace("Shutting down P2IO USB Interrupt thread...");
		USBThread.Wait();
		LOG->Trace("P2IO USB interrupt  thread shut down.");
	}

	if (USBBulkThread.IsCreated())
	{
		LOG->Trace("Shutting down P2IO USB Bulk thread...");
		USBBulkThread.Wait();
		LOG->Trace("P2IO USB Bulk thread shut down.");
	}
	...
	if (m_bFoundUSBDevice)
	{
		memset(InputHandler_P2IO::usb_light_message, 0, 5 * sizeof(uint8_t));// zero out the message
		Board.writeLights(InputHandler_P2IO::usb_light_message);
		Board.Close();
...
int InputHandler_P2IO::USBBulkThreadHD_Start(void *p)
{
	((InputHandler_P2IO *)p)->USBBulkThreadMainHD();
	return 0;
}
int InputHandler_P2IO::USBBulkThreadSD_Start(void *p)
{
	((InputHandler_P2IO *)p)->USBBulkThreadMainSD();
	return 0;
}
void InputHandler_P2IO::USBBulkThreadMainHD()
{
	red_top_count=0;
	red_bottom_count=0;
	blue_top_count=0;
...
void InputHandler_P2IO::USBBulkThreadMainSD()
{
	while (!m_bShutdown)
	{
		UpdateLightsUSBSD();
...
int InputHandler_P2IO::USBThread_Start(void *p)
{
	((InputHandler_P2IO *)p)->USBThreadMain();
	return 0;
}
void InputHandler_P2IO::USBThreadMain()
{
	...
	{
		LOG->Warn("Failed to set P2IO thread priority: %d", GetLastError());
	}
	...
	while (!m_bShutdown)
	{
		...
		//LOG->Info( "P2IO::about to process" );
		if (!Board.interruptRead(packetBuffer))
		{
			LOG->Warn("P2IO disconnected? Trying to reconnect");
			Board.Reconnect();
			usleep(100000);
			continue;
		}
...
void InputHandler_P2IO::ProcessDataFromDriver()
{
	//LOG->Info( "P2IO::Processing data from driver" );
	int i = 0;
	for (i = 0; i<P2IO_NUM_STATES; i++)
	{
		int byte = i / 8;
		int bit = i % 8;
		if (BYTE_BIT_IS_SET_M_TO_L(packetBuffer[byte], bit) != previousStates[i])
		{
			...
			//LOG->Info( "P2IO input state of button %d changed to %d",i,previousStates[i] );
#ifdef PRODUCT_ID_BARE
#ifndef STDSTRING_H
			ButtonPressed(DeviceInput(DEVICE_KEYBOARD, button_list_dupe[i], previousStates[i]));
#else
			ButtonPressed(DeviceInput(DEVICE_KEYBOARD, button_list_dupe[i], previousStates[i]));
...
void InputHandler_P2IO::UpdateLightsUSBSD()
{
	...
	if (ls.m_bGameButtonLights[GameController_1][GAME_BUTTON_START]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_LR;  //on hd cabinet buttons left LR is red spotlights satellite bottom
	if (ls.m_bGameButtonLights[GameController_1][GAME_BUTTON_START]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_UD; //on hd cabinet buttons left UD is red spotlights marquee top
	if (ls.m_bGameButtonLights[GameController_2][GAME_BUTTON_START]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_LR;//on hd cabinet buttons right LR is blue spotlights satellite bottom
	if (ls.m_bGameButtonLights[GameController_2][GAME_BUTTON_START]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_UD;//on hd cabinet buttons right UD is blue spotlights marquee top
	...
	if (ls.m_bCabinetLights[LIGHT_MARQUEE_LR_RIGHT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_MARQUEE_LR; //in hd mode this is different
	if (ls.m_bCabinetLights[LIGHT_MARQUEE_UP_RIGHT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_MARQUEE_UR; //in hd mode this is different
	if (ls.m_bCabinetLights[LIGHT_MARQUEE_LR_LEFT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_MARQUEE_LL; //in hd mode this is different
	if (ls.m_bCabinetLights[LIGHT_MARQUEE_UP_LEFT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_MARQUEE_UL; //in hd mode this is different
#else
	
	const LightsState *ls = LightsDriver_External::Get();
	//i am convinced the actual button lights are controlled over serial on HD cabinets
	if (ls->m_bCabinetLights[LIGHT_BUTTONS_LEFT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_LR;  //on hd cabinet buttons left LR is red spotlights satellite bottom
	if (ls->m_bCabinetLights[LIGHT_BUTTONS_LEFT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_UD; //on hd cabinet buttons left UD is red spotlights marquee top
	if (ls->m_bCabinetLights[LIGHT_BUTTONS_RIGHT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_LR;//on hd cabinet buttons right LR is blue spotlights satellite bottom
	if (ls->m_bCabinetLights[LIGHT_BUTTONS_RIGHT]) InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_UD;//on hd cabinet buttons right UD is blue spotlights marquee top
...
void InputHandler_P2IO::UpdateLightsUSBHD()
{
	//simulate lights not updated here...
	memset(InputHandler_P2IO::usb_light_message, 0, 5 * sizeof(uint8_t));// zero out the message
	...
	if (lr_right)
	{
		InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_LR; //in hd mode this is different -- see above for why I do this
		red_bottom_count+=1;
	}
	else
	{
	...
	if (up_right)
	{
		InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_LR; //in hd mode this is different
		blue_bottom_count+=1;
	}
	else
	{
	...
	if (lr_left)
	{
			InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P1_PANEL_UD; //in hd mode this is different
			red_top_count+=1;
	}
	else
	{
	...
	if (up_left)
	{
		InputHandler_P2IO::usb_light_message[0] |= USB_OUT_P2_PANEL_UD; //in hd mode this is differnt
		blue_top_count+=1;
	}
	else
	{
...
uint8_t InputHandler_P2IO::capAt(uint8_t cap,uint8_t var)
{
