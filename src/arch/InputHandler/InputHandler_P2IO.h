#pragma once
#ifndef INPUT_HANDLER_P2IO_H
#define INPUT_HANDLER_P2IO_H
...
class InputHandler_P2IO : public InputHandler
{
public:
	InputHandler_P2IO();
	~InputHandler_P2IO();
...
private:
	...
	* we need this to prevent obscure I/O problems. */
    ...
    static uint8_t usb_light_message[];



#define P2IO_NUM_STATES 22
#ifndef PRODUCT_ID_BARE
	const static int button_list[P2IO_NUM_STATES];
#else
	#ifndef STDSTRING_H

	#else
	
	const static DeviceButton button_list[P2IO_NUM_STATES];
	#endif
#endif
	bool previousStates[P2IO_NUM_STATES];
