...
static Preference<PSTRING> m_COM4PORT("P2IOCOM4PORT", ""); //COM2
...
uint8_t previousChunk[3][4];

// p2io correlated pid/vid
const uint16_t P2IO_VENDOR_ID[1] = { 0x0000 };
const uint16_t P2IO_PRODUCT_ID[1] = { 0x7305 };

uint8_t previous_light_payload[] = { 0, 0, 0, 0, 0 };
const unsigned NUM_P2IO_CHECKS_IDS = ARRAYLEN(P2IO_PRODUCT_ID);
int interrupt_endpoint = 0x83;
int bulk_write_to_endpoint = 0x02;
int bulk_read_from_endpoint = 0x81;
...
int bulkReplySize=0;
bool baudPass=false;
bool hdxbReady=false;



bool P2IO::DeviceMatches(int iVID, int iPID)
{
	for (unsigned i = 0; i < NUM_P2IO_CHECKS_IDS; ++i)
	{
		if (iVID == P2IO_VENDOR_ID[i] && iPID == P2IO_PRODUCT_ID[i])
		{
			return true;
		}
	}
...
bool P2IO::Open()
{
	...
	for (unsigned i = 0; i < NUM_P2IO_CHECKS_IDS; ++i)
		if (OpenInternal(P2IO_VENDOR_ID[i], P2IO_PRODUCT_ID[i]))
		{
			LOG->Info("P2IO Driver:: Connected to index %d", i);
			connect_index = i;
			if (i == 2)
			{
				interrupt_endpoint = 0x81; //debug mouse
...
bool P2IO::interruptRead(uint8_t* dataToQueue)
{
	...
	while (!allChunks)
	{
		int iResult = m_pDriver->InterruptRead(interrupt_endpoint, (char*)tchunk, iExpected, REQ_TIMEOUT);
	...
	else //copy our current state to the previous state
	{
		for (int i = 0; i<3; i++)
		{
			for (int j = 0; j<4; j++)
			{
				previousChunk[i][j] = chunk[i][j];
			}
		}
	}

	//format the data in the way we expect -- harkens back to the UDP p2io streamer
	uint8_t DDR_P1_PAD_UP = checkInput(previousChunk[0][1], DDR_PAD_UP);
	uint8_t DDR_P1_PAD_DOWN = checkInput(previousChunk[0][1], DDR_PAD_DOWN);
	uint8_t DDR_P1_PAD_LEFT = checkInput(previousChunk[0][1], DDR_PAD_LEFT);
	uint8_t DDR_P1_PAD_RIGHT = checkInput(previousChunk[0][1], DDR_PAD_RIGHT);

	uint8_t DDR_P1_CP_UP = checkInput(previousChunk[0][3], DDR_CP_UP_P1);
	uint8_t DDR_P1_CP_DOWN = checkInput(previousChunk[0][3], DDR_CP_DOWN_P1);
	uint8_t DDR_P1_CP_LEFT = checkInput(previousChunk[0][1], DDR_CP_LEFT);
	uint8_t DDR_P1_CP_RIGHT = checkInput(previousChunk[0][1], DDR_CP_RIGHT);
	uint8_t DDR_P1_CP_SELECT = checkInput(previousChunk[0][1], DDR_CP_SELECT);

	uint8_t DDR_P2_PAD_UP = checkInput(previousChunk[0][2], DDR_PAD_UP);
	uint8_t DDR_P2_PAD_DOWN = checkInput(previousChunk[0][2], DDR_PAD_DOWN);
	uint8_t DDR_P2_PAD_LEFT = checkInput(previousChunk[0][2], DDR_PAD_LEFT);
	uint8_t DDR_P2_PAD_RIGHT = checkInput(previousChunk[0][2], DDR_PAD_RIGHT);

	uint8_t DDR_P2_CP_UP = checkInput(previousChunk[0][3], DDR_CP_UP_P2);
	uint8_t DDR_P2_CP_DOWN = checkInput(previousChunk[0][3], DDR_CP_DOWN_P2);
	uint8_t DDR_P2_CP_LEFT = checkInput(previousChunk[0][2], DDR_CP_LEFT);
	uint8_t DDR_P2_CP_RIGHT = checkInput(previousChunk[0][2], DDR_CP_RIGHT);
	uint8_t DDR_P2_CP_SELECT = checkInput(previousChunk[0][2], DDR_CP_SELECT);

	uint8_t DDR_OP_TEST = checkInput(previousChunk[0][3], DDR_TEST);
	uint8_t DDR_OP_SERVICE = checkInput(previousChunk[0][3], DDR_SERVICE);
	uint8_t DDR_OP_COIN1 = checkInput(previousChunk[0][3], DDR_COIN);
	//uint8_t DDR_OP_COIN2=checkInput(previousChunk[0][3],DDR_COIN);
...
bool P2IO::spamBaudCheck()
{
	...
	else
	{
		...
		for (int i=0;i<50;i++)
		{
			...
			readHDXB(0x40);
			if (bulkReplySize>=16 && p2io_response[1]==0x0F && p2io_response[4]==0x0B &&  p2io_response[5]==0xAA && p2io_response[6]==0xAA && p2io_response[7]==0xAA && p2io_response[8]==0xAA && p2io_response[9]==0xAA && p2io_response[10]==0xAA && p2io_response[11]==0xAA && p2io_response[12]==0xAA && p2io_response[13]==0xAA && p2io_response[14]==0xAA && p2io_response[15]==0xAA)
			{
...
bool P2IO::writeLights(uint8_t* payload)
{
	...
	if ( (memcmp(previous_light_payload, payload, sizeof(previous_light_payload))!=0) || (packets_since_keepalive == 0) )
	{	//if they dont match...
		
		//copy new payload to previous payload so I can check it next round
		memcpy(previous_light_payload, payload, sizeof(previous_light_payload));
...
bool P2IO::WriteToBulkWithExpectedReply(uint8_t* message, bool init_packet, bool output_to_log)
{
	...
	*/
	...
	{
		sprintf (debugMessage+(i*3), "%02X ", message2[i]);
	}
	//if(output_to_log) LOG->Info("Send %d bytes - %s", bytes_to_write, debugMessage);

	int iResult = m_pDriver->BulkWrite(bulk_write_to_endpoint, (char*)message2, bytes_to_write, REQ_TIMEOUT);
	...
	bulkReplySize=iResult;


	//prepare debug line
	debugMessage[0]=0;
	for (int i=0;i<bulkReplySize;i++)
	{
		sprintf (debugMessage+(i*3), "%02X ", response[i]);
	}
	//if(output_to_log) LOG->Info("Full response is %d/%d bytes - %s", bulkReplySize,response[1]+2, debugMessage);
	

	//if the unescaped data is too small according to the packet length byte...
	if (bulkReplySize<(response[1]+2))
	{
		...
		iResult += GetResponseFromBulk(response+iResult, response[1]-bulkReplySize, output_to_log,overrider); //do a potentially fragmented read
		bulkReplySize=iResult;
		...
		if(output_to_log) LOG->Info("2nd attempt full response is %d/%d bytes - %s", bulkReplySize,response[1]+2, debugMessage);
	}
	for (int i=0;i<bulkReplySize;i++)
	{
...
int P2IO::GetResponseFromBulk(uint8_t* response, int expected_response_length, bool output_to_log, bool force_override)
{
	...
	//int responseLength = m_pDriver->BulkRead(bulk_read_from_endpoint, (char*)response, 128, REQ_TIMEOUT);
	...
	while (totalBytesRead < (expected_response_length+2)) //sometimes a response is fragmented over multiple requests, try a another time
	{
		...
		int bytesReadThisRound = m_pDriver->BulkRead(bulk_read_from_endpoint, (char*)response2, 64, REQ_TIMEOUT);
		...
		{
			sprintf (debugMessage+(i*3), "%02X ", response2[i]);
		}
		//if(output_to_log) LOG->Info("Bulk read round %d is %d bytes - %s", num_reads, bytesReadThisRound, debugMessage);
		//END DEBUG
...
void P2IO::FlushBulkReadBuffer()
{
	...
	bulkReplySize=GetResponseFromBulk(response3, 3, true); //do a potentially fragmented read
}
